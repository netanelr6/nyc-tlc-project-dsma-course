...
def compute_psi(reference: np.ndarray, current: np.ndarray, bins: int = 10) -> float:
    ...
    if len(breakpoints) < 3:
        return 0.0  # too few unique values to bin
...
def detect_label_drift(reference_raw_df: pd.DataFrame,
    ...
    """
    Quantify label drift on the total_fare_amount.
    ...
    def _compute_target(df):
        return (df["total_amount"] - df["tip_amount"]).dropna().values
...
def plot_label_drift_distribution(reference_raw_df, current_raw_df,
    ...
    def _target(df):
        ...
        return (df["total_amount"] - df["tip_amount"]).dropna()
    ...
    # ── Left: KDE overlay ─────────────────────────────────────────────────────
    ax = axes[0]
    ref_vals.plot.kde(ax=ax, label=f"{ref_label}  (μ=${ref_mean:.2f})",
                      color="steelblue", linewidth=2)
    cur_vals.plot.kde(ax=ax, label=f"{cur_label}  (μ=${cur_mean:.2f})",
    ...
    ax.set_title("total_fare_amount -- KDE Overlay")
    ax.set_xlabel("Total fare amount (dollars)")
    ...
    ax2.set_title("total_fare_amount -- Layered Histogram (density)")
    ax2.set_xlabel("Total fare amount (dollars)")
    ax2.legend(fontsize=9)
    ax2.spines[["top", "right"]].set_visible(False)

    delta_mean = cur_mean - ref_mean
    plt.suptitle(
        f"Label Drift: total_fare_amount  "
        f"(Δμ = ${delta_mean:+.2f},  {ref_label} vs {cur_label})",
    ...
    if output_dir:
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        path = Path(output_dir) / "label_drift_total_fare.png"
        plt.savefig(path, dpi=150, bbox_inches="tight")
        print(f"  Plot saved -> {path}")
...
def plot_monthly_mae_curve(monthly_summary: pd.DataFrame, output_dir=None):
    ...
    if len(jan_rows):
        ...
        ax.axhline(
            ...
            label=f"Baseline MAE = ${jan_mae:.2f}",
        )
...
def run_monthly_drift_analysis(monthly_eval_df, reference_raw_df,
    ...
    for month_num in months:
        ...
        print(
            f"  {month_label:<8} ${concept['current_mae']:>7.2f} "
