...
RECENCY_WEIGHT = 3.0  # weight multiplier for recent rows
...
# Mapping of engineered features to the feature creation step functions that produce them
FEATURE_SOURCE_MAP = {
    ...
    "est_base_fare":                "_add_domain_features",
    "est_congestion_surcharge":     "_add_domain_features",
    "est_extra":                    "_add_domain_features",
    "est_airport_fee":              "_add_domain_features",
    "est_improvement_surcharge":    "_add_domain_features",
    "est_mta_tax":                  "_add_domain_features",
    "est_total_fare_without_tolls": "_add_domain_features",
...
def plot_mitigation_comparison(error_arrays: dict, output_dir=None):
    ...
    for i, (label, errors) in enumerate(error_arrays.items()):
        ax.hist(
            ...
            label=f"{label}  (MAE = ${maes[label]:.2f})",
        )

    ax.set_xlabel("Absolute Error (dollars, clipped at 30)")
    ...
    for label, mae in maes.items():
        if label == orig_label:
            print(f"  {label:<40} ${mae:>7.2f}  {'(reference)':>12}")
        else:
            pct = (orig_mae - mae) / orig_mae * 100 if orig_mae > 0 else 0
            print(f"  {label:<40} ${mae:>7.2f}  {pct:>+11.1f}%")
