...
Run this script to execute the complete end-to-end pipeline:
  ...
  Step 2   — Data Loading + Split Setup
  ...
  Step 8   — Error Analysis                  (per-sample errors, segment breakdowns in USD)
  Step 9   — Drift Detection                 (PSI + KS on Train vs Test, label/concept drift)
  Step 9.1 — Drift Detection (Evidently AI)  (data + target + concept drift on Dec 2024)
  Step 10  — Drift Mitigation                (Evidently-driven strategy, before/after in USD)
...
def run_pipeline(wandb_project=WANDB_PROJECT, sample_size=None):
    ...
    # Clean datasets
    print("Cleaning training datasets...")
    ...
    # ── Step 2: Data Splitting / Setup ─────────────────────────────────────────
    _print_header("STEP 2 — Data Setup")
    ...
    baseline_test, _ = run_baseline_pipeline(
        test_clean, scaler=baseline_scaler, is_training=False
    )
    ...
    # Save the fitted scaler so the Streamlit app/pipeline can load it
    ...
    print(f"  Running 15 trials locally/via W&B sweep ...")
    ...
    print(f"  Best CV MAE       : ${best_random_mae:.2f}")
    print(f"  Best config       : {best_random_config}")
    print(f"  Sweep / Search ID : {random_sweep_id}")
    ...
    grid_sweep_id, best_grid_config, best_grid_mae = run_wandb_sweep(
        ...
        project      = wandb_project,
        n_runs       = 8,   # grid sweeps run all combinations locally/via W&B
    )
    print(f"\n  Grid search complete.")
    print(f"  Best CV MAE : ${best_grid_mae:.2f}  (random search was ${best_random_mae:.2f})")
    ...
    # W&B Table — filter by segments in browser
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # STEP 9 — Drift Detection (Train vs. Test)
    # ══════════════════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # STEP 9.1 — Drift Detection using Evidently AI (Train vs. Dec 2024)
    # ══════════════════════════════════════════════════════════════════════════
    ...
    _print_header("STEP 9.1 — Drift Detection (Evidently AI) — Dec 2024")
    ...
    # Engineer features with the January scaler
    ...
    # Baseline: tuned champion evaluated on Dec last week (pre-mitigation)
    ...
    # Re-engineer the Dec eval set with the updated scaler/features
    ...
    # Version the evaluation dataset artifact
    log_data_artifact(
    ...
    # Attach Evidently HTML report to W&B
...
if __name__ == "__main__":
    ...
    run_pipeline(wandb_project=args.wandb_project, sample_size=args.sample_size)
